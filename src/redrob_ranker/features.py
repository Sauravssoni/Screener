from src.redrob_ranker.text import count_terms, has_exact_match, normalize_text

CORE_AI_TERMS = [
    "embeddings", "vector search", "semantic search", "hybrid search", 
    "retrieval", "ranking", "recommender systems", "search relevance", 
    "information retrieval", "faiss", "milvus", "qdrant", "pinecone", 
    "weaviate", "elasticsearch", "opensearch", "bm25", "ann", "hnsw", 
    "rag", "sentence-transformers", "bge", "e5", "llm ranking", "learning-to-rank"
]

PRODUCTION_TERMS = [
    "production", "deployed", "real users", "latency", "monitoring", 
    "online", "a/b testing", "index refresh", "serving", "mlops", 
    "on-call", "feature pipeline", "quality regression", "model deployment"
]

EVALUATION_TERMS = [
    "ndcg", "map", "mrr", "ranking metrics", "offline benchmark", 
    "online experiment", "a/b test", "relevance labels", "recruiter feedback", 
    "click model", "evaluation framework"
]

PYTHON_SYSTEMS_TERMS = [
    "python", "fastapi", "pandas", "numpy", "pytorch", "scikit-learn", 
    "airflow", "spark", "kafka", "docker", "kubernetes", "aws", "gcp", 
    "distributed systems", "large-scale inference", "batch pipelines"
]

NEGATIVE_TERMS = [
    "research-only", "academic lab", "tutorial", "demo", "prototype", "kaggle"
]
...
def extract_features(candidate):
    ...
    # 1. Core AI Retrieval
    features['core_ai_count'] = count_terms(full_text, CORE_AI_TERMS) + sum(1 for s in skill_names if any(t in s for t in CORE_AI_TERMS))
    ...
    # 4. Python/Systems
    features['python_systems_count'] = count_terms(full_text, PYTHON_SYSTEMS_TERMS) + sum(1 for s in skill_names if any(t in s for t in PYTHON_SYSTEMS_TERMS))
    ...
    features['is_manager_only'] = 1 if 'manager' in curr_title and not any(t in curr_title for t in ['engineer', 'data', 'applied']) else 0
    ...
    features['ideal_location'] = 1 if any(city in loc for city in ['pune', 'noida', 'delhi', 'gurgaon', 'bangalore', 'bengaluru']) else 0
