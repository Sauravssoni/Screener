...
PYTHON_SYSTEMS_TERMS = ["python", "sql", "spark", "kafka", "redis", "postgres", "snowflake", "dbt", "go", "c++"]
...
def extract_features(candidate):
    ...
    features["is_manager_only"] = (
        1
        if any(m in curr_title for m in MANAGER_ROLES)
        and not any(t in curr_title for t in ["engineer", "data", "applied", "ml", "scientist"])
        else 0
    )
    features["is_research_only"] = 1 if extract_matched_terms(full_text, RESEARCH_TERMS) else 0
    features["is_prompt_only"] = (
        1 if extract_matched_terms(full_text, PROMPT_TERMS) and features["core_ai_count"] < 2 else 0
    )
