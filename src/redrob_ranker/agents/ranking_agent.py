...
def compute_career_relevance(candidate):
    ...
    for role in history[:3]:
        ...
        if any(kw in title or kw in desc for kw in ["machine learning", "ai", "data scientist", "nlp", "llm", "recommendation"]):
            score += weight
        weight *= 0.6
...
def rank_candidates(candidates_with_context):
    ...
    for i, (candidate, features, traps, evidence) in enumerate(candidates_with_context):
        ...
        if "customer support" in title or "accountant" in title or "civil engineer" in title or "operations manager" in title or "content writer" in title or "hr" in title or "sales" in title or "marketing" in title or "teacher" in title or "recruiter" in title:
            struct_score = 0.0
